...
Matrix AffineLayer::feedforward(const Matrix &X) {
  ...
  //Last_y와 Last_x는 이 X와 Y를 등록하는 것이다.
    Last_y = (X^W) + b;
    PlusStack += Last_y.PreviousPlusStack;
    MultiplyStack += Last_y.PreviousMultiplyStack;
    Last_x = X;
    return Last_y;
}
...
Matrix AffineLayer::feedbackward(const Matrix &input) {//역전파법
  ...
  //dW = X(T) ^ input
    ...
    PlusStack += Dw.PreviousPlusStack;
    MultiplyStack += Dw.PreviousMultiplyStack;
    ...
    Matrix tmp = input ^ W.GetInverse();
    PlusStack += tmp.PreviousPlusStack;
    MultiplyStack += tmp.PreviousMultiplyStack;
...
int NeuralNetwork::GetPlusStacks() const{
    int a =0;
    for(auto Block : Blocks)
    {
        a += Block.GetAffine().PlusStack;
    }
    return a;
}

int NeuralNetwork::GetMultiplyStacks() const{
    int a=0;
    for(auto Block : Blocks)
    {
        a += Block.GetAffine().MultiplyStack;
    }
    return a;
}
