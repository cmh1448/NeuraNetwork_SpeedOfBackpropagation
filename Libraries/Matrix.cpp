...
Matrix Matrix::operator^ (const Matrix &B) const//내적을 구하는 연산자 원래는 비트연산자이지만, 편의를 위해 사용하였다.
{
    ...
    for(size_t i=0; i< A.size().first; i++)
    {
        ...
        for(size_t j=0; j< B.size().second; j++)
        {
            ...
            {
                C.PreviousPlusStack += 1;
                C.PreviousMultiplyStack += 1;
                C[i][j] += A[i][l] * B[l][j];//더하기 대입
            }
        }
    }
    C.PreviousPlusStack += A.PreviousPlusStack + B.PreviousPlusStack;
    C.PreviousMultiplyStack += A.PreviousMultiplyStack + B.PreviousMultiplyStack;
    return C;
}
...
Matrix Matrix::operator+(const double &copy) const//double형과 같이하는용
{
    ...
    C.PreviousMultiplyStack += this->PreviousMultiplyStack;
    C.PreviousPlusStack += this->PreviousPlusStack;
    return C;
}
...
Matrix operator+(const double &copy, const Matrix &Mtrx)//교환법칙의 성립을 위해 작성
{
    ...
    C.PreviousPlusStack += Mtrx.PreviousPlusStack;
    C.PreviousMultiplyStack += Mtrx.PreviousMultiplyStack;
    return C;
}
//------------------------------------------------------------------------------
